...
public class Controller {
	...
	public boolean gameStarter() {
		...
		for(int i=0; i<num; i++) {
			...
			if(isDupl.contains(name)) {
				System.out.println("중복된 이름은 사용이 불가능해요");
				i--;
			}else {
				isDupl.add(name);
	...
	public void gamePlay() {
		...
		for(int i=0; i<playerList.size(); i++) {
			System.out.println("플레이어" + (playerList.get(i).getPlayerId()+1) + ")" + playerList.get(i).getName() + "님 " + checkMoney(playerList.get(i)) + "만원");
		}
	...
	public void sellHotel(Player p, int pay) {
		...
		// 가장 싼 선물부터 매각하기 시작
		System.out.println(p.getName() + "님의 가장 싼 호텔부터 매각하기 시작합니다.");
		int money = p.getMoney();
		while(money <= pay) {
			// 맨 앞의 값을 계속해서 뺌
			System.err.println(landList.get(p.getHotelList().get(0).getHotelPo()).getLandName()+ "의 호텔을 판배합니다. ");
			// 매각할 호텔의 돈 누적하기
	...
	private void eventCity(Player p) {
		// 출력문구
		System.out.println(landList.get(p.getPlayerPo()).getLandName() + "에 도착했습니다!");
		// 이벤트 내용 - 1. 소유X 2. 소유O 
		// 1-1. 땅을 살 것인지 
		...
		if(isTrap && !(trapOwner.equals(p.getName()))) {
			...
			System.out.println("이런, " + trapOwner + "님의 땅이네요!");
	...
	// (위치이동이벤트)황금열쇠 이벤트 *****
		private void eventGoldenKey(Player p) {
			...
			switch(keySelect) {
			case 1: charge(p, 5);
				System.out.println(p.getName()+"의 병원비로 지갑에서 5만원이 털렸습니다.");
				break;
			case 2: p.setMoney(money + 20);
				System.out.println(p.getName()+"20만원 복권에 당첨됐습니다. 저도 되고싶습니다.");
				break;
			case 3: movePosition(p, 14);
				System.out.println(p.getName()+"는 세계여행에 당첨되었습니다.");
				eventTravel(p);	
				break;
			case 4: charge(p, 10);
				System.out.println(p.getName()+"이 호그와트로 유학을 떠납니다. 입학비 10만원 납부하세요.");
				break;
			case 5: p.setPlayerPo(position + 4);
				System.out.println(p.getName()+"4칸 앞으로 전진!!");
				break;
			case 6: movePosition(p, 0);
				System.out.println(p.getName()+"고속도로를 타고 집으로 갑니다. 저도 데러가주세요...");
				break;
			case 7: eventBrith(p);
				break;
			case 8: p.setMoney(money + 10);
				System.out.println(p.getName()+"님, 교장 덤블도어에게 포상 10만원을 받았습니다. ");
				break;
			case 9: charge(p, p.getHotelList().size()*5);
				System.out.println(p.getName()+"건물주님, 세금 내셔야죠. "+p.getHotelList().size()+"채 : 5만원씩 걷겠습니다.");
				break;
			case 10: p.setMoney(money + 15);
				System.out.println(p.getName()+"님, 휴가비 15만원 받아가세요.");
				break;
		...
		private void eventBrith(Player p) {
			// 출력
			System.out.println(p.getName()+"의 생일입니다. \r\n"
					+ "당신은 다른플레이들에게 생일빵을 맞고 치료비로 5만원씩 받았습니다. ");
			// 이벤트 내용
			int expense = 5; // 뜯을 돈
			int totalExpenses = 0;
			...
			System.out.println(p.getName() + "님에게 " + totalExpenses + "만원 증정!!!");
...
//		int DesPoint = 0; // 도착 위치
		...
		if(DesPoint == 14) { // 📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌
			System.out.println("세계여행 지정 불가능해요! 출발지로 보내드릴게요!");
			DesPoint = 0;
		}// 📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌📌
		System.out.println("네, 다음 턴에 이동하겠습니다!");
//		// (2) 이동 거리 계산
//		if(dtNum < position) {
	...
	public int setInput() {
		while(true) {
			try {
				...
				scan.nextLine();
				System.out.println("숫자만 입력 가능해요! 다시 입력해주세요 :)");
			}
		}
	}
